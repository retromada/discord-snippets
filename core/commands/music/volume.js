...
module.exports = {
  ...
  execute(message) {
    ...
    if (!amount) return message.channel.send(new MessageEmbed().setDescription(`Current volume is **${player.volume}%** (${(player.connection.dispatcher.volumeLogarithmic).toFixed(1)})`))
